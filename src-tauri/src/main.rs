...
fn main() {
    ...
    let app = Arc::new(rt.block_on(async { build_platform_app(None).await }).expect("failed to build app"));
...
async fn build_platform_app(permission_ui: Option<Arc<dyn PermissionUi>>) -> anyhow::Result<OpenPupApp> {
