...
impl MemorySystem {
  ...
  async fn init_schema(&self) -> Result<()> {
    ...
    let _ = sqlx::query(
      "ALTER TABLE conversations ADD COLUMN pup TEXT NOT NULL DEFAULT 'alpha'",
    )
    .execute(&self.pool)
    .await;
    ...
    sqlx::query(
      "CREATE INDEX IF NOT EXISTS idx_conversations_pup ON conversations(pup, id)",
    )
    .execute(&self.pool)
    .await?;
  ...
  pub async fn complete_channel(&self, channel_id: &str) -> Result<()> {
    sqlx::query(
      "UPDATE pack_channels SET status='completed', completed_at=?1 WHERE id=?2",
    )
    .bind(Utc::now().timestamp())
    .bind(channel_id)
    .execute(&self.pool)
      ...
      rows
        ...
        .map(|row| {
          ...
          let members: Vec<String> =
            members_str.split(',').filter(|s| !s.is_empty()).map(|s| s.to_string()).collect();
          ChannelRecord {
            id: row.get("id"),
  ...
  pub async fn get_channel_messages(&self, channel_id: &str) -> Result<Vec<ChannelMessageRecord>> {
    let rows = sqlx::query(
      r#"
    ...
    Ok(
      rows
        .into_iter()
        .map(|row| {
          let mentions_json: String = row.get::<Option<String>, _>("mentions").unwrap_or_default();
  ...
  pub async fn search_long_term(&self, query: &str, limit: usize) -> Result<Vec<String>> {
    ...
    let query_vec = match self.llm.embed(query).await {
      ...
      Err(_) => {
        ...
        .fetch_all(&self.pool)
        .await?;
        return Ok(rows.into_iter().map(|r| r.get::<String, _>("content")).collect());
      }
    };
  ...
  pub async fn conversation_count(&self) -> Result<i64> {
    let row =
      sqlx::query("SELECT COUNT(*) as cnt FROM conversations").fetch_one(&self.pool).await?;
    Ok(row.get::<i64, _>("cnt"))
  }
...
impl MemorySystem {
  ...
  pub async fn update_task_status(
    ...
    let completed_at: Option<i64> =
      if status == "done" || status == "failed" { Some(Utc::now().timestamp()) } else { None };
    sqlx::query(
...
impl MemorySystem {
  ...
  pub async fn has_similar_memory(&self, text: &str, min_similarity: f32) -> bool {
    ...
    let rows = match sqlx::query(
      r#"SELECT embedding FROM memory_embeddings LIMIT 500"#,
    )
    .fetch_all(&self.pool)
    .await {
      Ok(r) => r,
...
impl MemorySystem {
  ...
  pub async fn get_context_summary(&self, pup: &str) -> Result<Option<(String, i64)>> {
    let row = sqlx::query(
      "SELECT summary, covers_through_row FROM context_summaries WHERE pup = ?1",
    )
    .bind(pup)
    .fetch_optional(&self.pool)
    .await?;
    Ok(row.map(|r| (r.get::<String, _>("summary"), r.get::<i64, _>("covers_through_row"))))
  }
  ...
  pub async fn conversations_after_row(
    ...
    .fetch_all(&self.pool)
    .await?;
    Ok(rows.into_iter().map(|r| (r.get("id"), r.get("role"), r.get("content"))).collect())
  }

  /// Return the max rowid for a pup's conversations (0 if none).
  pub async fn max_conversation_row(&self, pup: &str) -> Result<i64> {
    let row = sqlx::query(
      "SELECT COALESCE(MAX(id), 0) as m FROM conversations WHERE pup = ?1",
    )
    .bind(pup)
    .fetch_one(&self.pool)
    .await?;
