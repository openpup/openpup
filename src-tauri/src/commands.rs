...
use crate::mcp::orchestrator::{McpServerEntry, McpToolInfo, MCPOrchestrator};
...
pub async fn send_message(
  ...
  let (_, model, mini_model, _embed_model, api_key, api_base) = state.alpha.llm_client.current_config();
  eprintln!(
  ...
  tauri::async_runtime::spawn(async move {
    alpha.process_user_message_stream(input, forced_pup, &app_handle).await;
  });
  Ok(())
}
...
pub async fn save_onboarding_data(
  ...
  let _ = memory
    .add_long_term_memory(&format!("行为边界：{}", data.boundaries.trim()), "rule", 0.99)
    .await;
  ...
  if !data.language.trim().is_empty() {
    let _ = memory
      .add_long_term_memory(&format!("语言偏好：{}", data.language.trim()), "preference", 0.95)
      .await;
  }
  if !data.name.trim().is_empty() {
    let _ = memory
      .add_long_term_memory(&format!("用户名字/称呼：{}", data.name.trim()), "preference", 0.99)
      .await;
  }
...
pub async fn list_long_term_memories(
  ...
  Ok(rows
    .into_iter()
    .map(|(id, content, memory_type, importance, created_at)| LongTermMemoryItem {
      id,
      content,
      memory_type,
      importance,
...
pub async fn get_top_memories(
  ...
  let memory = memory_system_from_state(&state);
  let rows = memory.get_top_memories(limit).await.map_err(|e| e.to_string())?;
  Ok(rows
    .into_iter()
...
pub async fn uninstall_skill(state: State<'_, AppState>, name: String) -> Result<(), String> {
  registry(&state).uninstall(&name).await.map_err(|e| e.to_string())
}
...
pub async fn set_skill_enabled(
  ...
  registry(&state).set_enabled(&name, enabled).await.map_err(|e| e.to_string())
}
...
pub async fn run_skill(
  ...
  state.alpha.skill_executor.execute_skill(&name, &input).await.map_err(|e| e.to_string())
}
...
pub async fn list_mcp_servers(state: State<'_, AppState>) -> Result<Vec<McpServerInfo>, String> {
  ...
  Ok(servers.into_iter().map(|e| McpServerInfo {
    name: e.name,
    base_url: e.base_url,
...
pub async fn add_mcp_server(
  ...
  mcp(&state).add_server(McpServerEntry {
    name: entry.name,
...
pub async fn remove_mcp_server(state: State<'_, AppState>, name: String) -> Result<(), String> {
  mcp(&state).remove_server(&name).await.map_err(|e| e.to_string())
}
...
pub async fn toggle_mcp_server(
  ...
  mcp(&state).toggle_server(&name, enabled).await.map_err(|e| e.to_string())
}
...
pub async fn remove_custom_pup(state: State<'_, AppState>, key: String) -> Result<(), String> {
  state.alpha.remove_custom_pup(&key).await.map_err(|e| e.to_string())
}
...
pub async fn list_skill_runs(
  ...
  let memory = memory_system_from_state(&state);
  let rows = memory.list_skill_runs(limit).await.map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn read_diary_entry(
  state: State<'_, AppState>,
  date: String,
) -> Result<String, String> {
  state.file_layer.read_diary(&date).map_err(|e| e.to_string())
}
...
pub async fn get_llm_config(state: State<'_, AppState>) -> Result<LlmConfigInfo, String> {
  ...
  Ok(LlmConfigInfo { provider: provider_str, model, mini_model, embed_model, api_base })
}
...
pub async fn set_llm_provider(
  ...
  state.alpha.llm_client.reconfigure(
    p, model.clone(), mini_model.clone(), embed_model.clone(), api_key.clone(), api_base.clone(),
  );
  ...
  cfg.llm.model = model;
  if let Some(mm) = mini_model  { cfg.llm.mini_model = mm; }
  if let Some(em) = embed_model { cfg.llm.embed_model = em; }
  if let Some(k)  = api_key     { cfg.llm.api_key = k; }
  if let Some(b)  = api_base    { cfg.llm.api_base = b; }
...
pub async fn quick_set_model(state: State<'_, AppState>, model: String) -> Result<(), String> {
  ...
  let mut cfg = crate::config::load();
  cfg.llm.provider = match provider { Provider::OpenAI => "openai", Provider::Ollama => "ollama" }.to_string();
  cfg.llm.model = model;
  if let Some(k) = api_key  { cfg.llm.api_key = k; }
  if let Some(b) = api_base { cfg.llm.api_base = b; }
  crate::config::save(&cfg).map_err(|e| e.to_string())?;
...
pub async fn search_conversations(
  ...
  let memory = memory_system_from_state(&state);
  let rows =
    memory.search_conversations(&query, limit).await.map_err(|e| e.to_string())?;
  Ok(rows
    .into_iter()
    .map(|(role, content, timestamp)| ConversationSearchResult { role, content, timestamp })
    .collect())
}
...
pub async fn fetch_skill_manifest(
  ...
  let client = reqwest::Client::builder()
    ...
    .build()
    .map_err(|e| e.to_string())?;
  let resp = client.get(&raw_url).send().await.map_err(|e| e.to_string())?;
  if !resp.status().is_success() {
    return Err(format!("HTTP {} fetching manifest: {}", resp.status(), raw_url));
  }
...
fn github_to_raw_url(repo_url: &str, subdir: Option<&str>) -> Result<String, String> {
  ...
  if !base.contains("github.com") {
    return Err(format!("fetch_skill_manifest only supports GitHub URLs, got: {repo_url}"));
  }
...
pub async fn list_channels(
  ...
  state.alpha.memory.list_channels(50).await.map_err(|e| e.to_string())
}
...
pub async fn get_channel_messages(
  ...
  state.alpha.memory.get_channel_messages(&channel_id).await.map_err(|e| e.to_string())
}
...
pub async fn get_pup_conversation(
  ...
  let rows = state.alpha.memory
    .get_pup_conversation_display(&pup_key, limit)
    .await
    .map_err(|e| e.to_string())?;
  Ok(rows.into_iter().map(|(role, content, timestamp)| ConvMessage { role, content, timestamp }).collect())
}
...
#[tauri::command]
pub async fn clear_pup_history(
  state: State<'_, AppState>,
  pup_key: String,
) -> Result<(), String> {
  state.alpha.memory
