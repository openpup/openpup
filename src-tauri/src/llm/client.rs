...
impl LlmClient {
  pub fn new_from_env() -> Self {
    ...
    let (model, mini_model, embed_model, api_key, api_base) = match provider {
      Provider::OpenAI => {
        ...
        let mini_model = std::env::var("OPENAI_MINI_MODEL")
      ...
      Provider::Ollama => {
        ...
        let mini_model = std::env::var("OLLAMA_MINI_MODEL")
          .unwrap_or_else(|_| model.clone());
  ...
  pub fn reconfigure(
    ...
    g.provider = provider;
    if let Some(mm) = mini_model { g.mini_model = mm; }
    if let Some(em) = embed_model { g.embed_model = em; }
    g.model = model;
    g.api_key = api_key;
    g.api_base = api_base;
  }

  pub fn current_config(&self) -> (Provider, String, String, String, Option<String>, Option<String>) {
  ...
  pub async fn chat_mini(&self, messages: Vec<LlmMessage>) -> Result<String> {
    let model = {
      let g = self.config.read().unwrap();
      if g.mini_model.is_empty() { g.model.clone() } else { g.mini_model.clone() }
    };
    self.chat_with_model(&model, messages).await
  }
  ...
  pub async fn chat_stream(
    ...
    let model = self.config.read().unwrap().model.clone();
    self.stream_with_model(&model, messages, on_token, abort).await
  }
  ...
  async fn stream_with_model(
    ...
    'outer: loop {
      ...
      if abort.load(Ordering::Relaxed) {
        eprintln!("[llm] stream aborted after {chunk_count} tokens ({} chars)", full.len());
        break;
      }
      ...
      match next {
        ...
        Ok(None) => {
          eprintln!("[llm] stream complete: {chunk_count} tokens, {} chars", full.len());
          break;
        }
  ...
  pub async fn chat_with_tools(
    ...
    let url = chat_url(&api_base);
    eprintln!("[llm] chat_with_tools: model={model:?} tools={}", tools.len());
    ...
    let resp = req.send().await.map_err(|e| anyhow!("chat_with_tools request: {e}"))?;
    if !resp.status().is_success() {
      let status = resp.status();
      let body = resp.text().await.unwrap_or_default();
    ...
    let content = message["content"].as_str().filter(|s| !s.is_empty()).map(|s| s.to_string());
    ...
    Ok(ChatWithToolsResponse { content, tool_calls, raw_message: message })
  }
  ...
  pub async fn embed(&self, text: &str) -> Result<Vec<f32>> {
    ...
    let resp = req.send().await.map_err(|e| anyhow!("embed request: {e}"))?;
    if !resp.status().is_success() {
      let status = resp.status();
      let body = resp.text().await.unwrap_or_default();
