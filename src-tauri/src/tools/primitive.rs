...
impl ToolRegistry {
  ...
  pub async fn execute(
    ...
    match name {
      "shell_exec" => {
        if !perms.shell {
          return Err(anyhow!("shell_exec: permission denied (skill requires permissions.shell = true)"));
        }
        let cmd = args["command"].as_str().ok_or_else(|| anyhow!("shell_exec: missing 'command'"))?;
      ...
      "file_read" => {
        ...
        let path = args["path"].as_str().ok_or_else(|| anyhow!("file_read: missing 'path'"))?;
      ...
      "file_write" => {
        ...
        let path = args["path"].as_str().ok_or_else(|| anyhow!("file_write: missing 'path'"))?;
        let content = args["content"].as_str().ok_or_else(|| anyhow!("file_write: missing 'content'"))?;
        self.file_write(path, content).await
      }
      "http_get" => {
        if !perms.network {
          return Err(anyhow!("http_get: permission denied (skill requires permissions.network = true)"));
        }
        let url = args["url"].as_str().ok_or_else(|| anyhow!("http_get: missing 'url'"))?;
        self.http_get(url).await
      }
      "memory_search" => {
        let query = args["query"].as_str().ok_or_else(|| anyhow!("memory_search: missing 'query'"))?;
      ...
      "memory_store" => {
        let content = args["content"].as_str().ok_or_else(|| anyhow!("memory_store: missing 'content'"))?;
        self.memory.add_long_term_memory(content, "skill", 0.6).await?;
  ...
  async fn shell_exec(&self, command: &str) -> Result<String> {
    ...
    if trimmed.len() > 16_384 {
      Ok(format!("{}\n… [truncated, {} chars total]", &trimmed[..16_384], trimmed.len()))
    } else {
      Ok(trimmed)
    }
  }
  ...
  async fn file_read(&self, path: &str) -> Result<String> {
    ...
    if content.len() > 32_768 {
      Ok(format!("{}\n… [truncated, {} chars total]", &content[..32_768], content.len()))
    } else {
      Ok(content)
    }
  }
  ...
  async fn file_write(&self, path: &str, content: &str) -> Result<String> {
    let resolved = self.resolve_path(path);
    eprintln!("[tool/file_write] {} ({} bytes)", resolved.display(), content.len());
    ...
    Ok(format!("Written {} bytes to '{}'", content.len(), resolved.display()))
  }
  ...
  async fn http_get(&self, url: &str) -> Result<String> {
    ...
    if body.len() > 8_192 {
      Ok(format!("{}\n… [truncated, {} chars total]", &body[..8_192], body.len()))
    } else {
      Ok(body)
    }
  }
  ...
  fn resolve_path(&self, path: &str) -> PathBuf {
    ...
    if p.is_absolute() { p.to_path_buf() } else { self.workspace_root.join(path) }
  }
}
