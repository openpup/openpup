...
pub fn format_progress(done_pups: &[String], layer_index: usize) -> String {
    ...
    let items = done_pups.iter().map(|p| format!("\u{2713} {}", p)).collect::<Vec<_>>().join("\n");
    format!("Layer {} done:\n{}", layer_index + 1, items)
}
...
pub fn format_perm_request(action_desc: &str, skill: &str) -> String {
    format!("\u{26a0}\u{fe0f} {} needs your permission\n\n{}\n\nReply yes to allow, no to deny", skill, action_desc)
}
