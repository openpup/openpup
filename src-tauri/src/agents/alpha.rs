...
impl AlphaPup {
  ...
  pub async fn process_user_message_stream(
    ...
    eprintln!("[alpha] process_user_message_stream: msg_len={} forced_pup={forced_pup:?}", msg.len());
    self.abort_flag.store(false, Ordering::Relaxed);

    let result = self.do_stream(&msg, forced_pup, app_handle).await;
    match result {
      Ok((reply, pup_key)) => {
        eprintln!("[alpha] do_stream ok: pup={pup_key:?} reply_len={}", reply.len());
        ...
        eprintln!("[alpha] emitting stream_done pup={} aborted={aborted}", pup_display_name(&pup_key));
        let _ = app_handle.emit("stream_done", StreamDonePayload {
          pup_name: pup_display_name(&pup_key),
          content: if aborted { String::new() } else { reply.clone() },
        });
        ...
        if !aborted && !reply.is_empty() {
          ...
          tauri::async_runtime::spawn(async move {
            ...
            let _ = self_clone.memory.add_conversation(&pup_key_clone, "user", &msg_clone).await;
            let _ = self_clone.memory.add_conversation(&pup_key_clone, "assistant", &reply).await;
            ...
            let ellipsis = if msg_clone.chars().count() > 80 { "…" } else { "" };
            ...
            self_clone.maybe_create_task(&msg_clone, &pup_key_clone).await;
          });
        }
      }
      Err(e) => {
  ...
  async fn do_stream(
    ...
    let pup_key = if let Some(forced) = forced_pup {
      ...
      self.classify_intent(msg, &owner_summary, &classify_history).await
    };
    eprintln!("[alpha] do_stream: pup_key={pup_key:?}");

    // Notify the UI which pup/skill is handling this request
    let _ = app_handle.emit("stream_activity", ActivityEvent {
      kind: "routing".into(),
    ...
    if let Some(pups_str) = pup_key.strip_prefix("channel:") {
      let required_pups: Vec<String> =
        pups_str.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
      if required_pups.len() >= 2 {
        let output = self.run_parallel_pack(msg, required_pups, app_handle).await?;
    ...
    if pup_key == "alpha" {
      ...
      let reply = self
        .alpha_reply_stream(msg, &owner_summary, &pup_history, &relevant_memories, &pending_tasks, app_handle)
        .await?;
    ...
    if let Some(pup) = pup {
      ...
      let output = self
        .run_agent_with_tools(
          ...
          msgs,
          &tool_perms,
          move |tok| { let _ = handle.emit("stream_token", tok); },
          move |kind, label| { let _ = handle2.emit("stream_activity", ActivityEvent { kind, label }); },
          &self.abort_flag,
        )
        .await?;
      return Ok((output, pup_key));
    ...
    let reply = self
      .alpha_reply_stream(msg, &owner_summary, &fallback_history, &relevant_memories, &pending_tasks, app_handle)
      .await?;
  ...
  async fn alpha_reply_stream(
    ...
    let tool_perms = PupToolPermissions { shell: false, filesystem: false, network: false, mcp: true };
  ...
  async fn run_agent_with_tools(
    ...
    if tool_perms.mcp {
      ...
      let task_hint = messages
        ...
        .unwrap_or("");
      let mcp_specs = self.mcp_orchestrator.tools_for_task(task_hint, Self::MAX_MCP_TOOLS).await;
      eprintln!("[{agent_name}] injecting {} MCP tools (filtered)", mcp_specs.len());
    ...
    for iter in 0..MAX_ITER {
      ...
      let response = self.llm_client.chat_with_tools(msgs.clone(), available_tools.clone()).await?;
    ...
    Err(anyhow!("[{agent_name}] exceeded maximum tool-call iterations ({MAX_ITER})"))
  }
  ...
  async fn run_parallel_pack(
    ...
    let pup_list = required_pups.iter().map(|k| pup_display_name(k)).collect::<Vec<_>>().join("、");
    ...
    let owner_summary =
      self.get_owner_summary(&self.file_layer.read_owner_profile().unwrap_or_default()).await;
    ...
    let _ = app_handle.emit(
      "stream_activity",
      ActivityEvent { kind: "routing".into(), label: format!("pack:{pup_list} → 汇总") },
    );
  ...
  async fn classify_intent(&self, msg: &str, owner_summary: &str, history: &[LlmMessage]) -> String {
    ...
    let enabled_pups: Vec<String> = {
      let cfgs = self.pup_configs.read().await;
      cfgs.values().filter(|c| c.enabled && !c.is_custom || (c.is_custom && c.enabled)).map(|c| c.key.clone()).collect()
    };
    let skill_entries = self.skill_executor.registry.enabled_skill_names_and_triggers().await;

    let pup_options: String = enabled_pups.iter().map(|k| format!(" | {k}")).collect();
    let skill_options: String = skill_entries.iter().map(|(n, _)| format!(" | skill:{n}")).collect();
    ...
    let snippet = if owner_summary.len() > 400 { &owner_summary[..400] } else { owner_summary };
    ...
    let mut classifier_msgs =
      vec![LlmMessage { role: "system".into(), content: system_prompt }];
  ...
  async fn get_owner_summary(&self, owner_md: &str) -> String {
    ...
    let summary = if owner_md.len() <= 800 {
      ...
      match self
        .llm_client
        .chat_mini(vec![LlmMessage { role: "user".into(), content: prompt }])
        .await
      {
        Ok(s) => s,
  ...
  async fn build_history(&self, pup: &str) -> Vec<LlmMessage> {
    ...
    let recent = self
      ...
      .await
      .unwrap_or_default();
    msgs.extend(recent.into_iter().rev().map(|(role, content)| LlmMessage { role, content }));
    msgs
  }
  ...
  async fn build_classify_history(&self) -> Vec<LlmMessage> {
    let recent = self
      ...
      .await
      .unwrap_or_default();
    recent.into_iter().rev().map(|(role, content)| LlmMessage { role, content }).collect()
  }
  ...
  async fn maybe_compress_context(&self, pup: &str) -> Result<()> {
    ...
    self.memory.save_context_summary(pup, &new_summary, last_row_id).await?;
    ...
    if let Ok(serde_json::Value::Array(items)) = serde_json::from_str(json_str) {
      for item in items {
        if let (Some(text), Some(mem_type)) = (
          ...
          let importance =
            item.get("importance").and_then(|v| v.as_f64()).unwrap_or(0.7) as f32;
          if self.memory.has_similar_memory(text, 0.88).await {
            continue;
          }
          let _ = self.memory.add_long_term_memory(text, mem_type, importance).await;
  ...
  async fn maybe_create_task(&self, msg: &str, pup_key: &str) {
    ...
    let looks_like_task = ["帮我", "帮助", "创建", "安排", "提醒", "记得", "todo",
      "task", "remind", "create", "schedule", "help me"]
      .iter()
      .any(|kw| msg.to_lowercase().contains(kw));
    if !is_specialist && !looks_like_task { return; }
