...
impl SkillScheduler {
  ...
  async fn tick_scheduled_skills(&self, app_handle: &AppHandle) {
    ...
    for skill in installed.iter().filter(|s| s.enabled) {
      ...
      let Some(sched_cfg) = &manifest.schedule else { continue };
      let Some(cron_expr) = &sched_cfg.cron else { continue };
      ...
      if let Some(next) = schedule.after(&window_start).next() {
        if next <= now {
          self.run_skill_with_history(&skill.name, "cron", "", app_handle).await;
        }
      }
    }
  }
  ...
  async fn tick_alpha_heartbeat(&self, app_handle: &AppHandle) {
    ...
    let recent_convs = self.memory.recent_conversations_global(40).await.unwrap_or_default();
    ...
    for line in response.lines() {
      let Some(rest) = line.trim().strip_prefix("RECOMMEND:") else { continue };
      ...
      match mode {
        ExecutionMode::FreeRun => {
          ...
          if self.executor.registry.install_from_git(&discovered.repo_url, None).await.is_ok() {
            self
              .run_skill_with_history(&skill_name, "heartbeat", "heartbeat auto-run", app_handle)
              .await;
          }
        }
    ...
    let _ = self.memory.record_skill_run(&run_id, "alpha_heartbeat", "heartbeat").await;
    let _ = self
      .memory
  ...
  async fn run_skill_with_history(
    ...
    let run_id = Uuid::new_v4().to_string();
    let _ = self.memory.record_skill_run(&run_id, skill_name, triggered_by).await;
    ...
    let _ = self.memory.complete_skill_run(&run_id, &status, &output).await;
