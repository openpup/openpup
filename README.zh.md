...
- **流式聊天** + 智能多智能体路由
- **长期记忆** — 基于内容信号与最小间隔自动提取事实、偏好与规则；Weibull 衰减加权检索，近期记忆优先但不遗忘；语义去重防止噪音累积。长期记忆全局共享，对话记忆 per-pup 独立
- **规则系统** — `RULES.md` 中的规则强制注入每次对话，确保小狗始终遵守你的边界和偏好
- **知识库** — 导入文档（PDF/TXT/MD）自动分块索引；知识型问题会轻量检索相关片段注入上下文，协作产出与对话摘要仅在内容足够值得沉淀时才自动入库
